...
class TablasNormativasMasterResolutionTests(unittest.TestCase):
    ...
    def test_hojas_equivalentes_no_bloquean_y_prefiere_la_mas_limpia(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            for code in ["5302-Z1", "5302-Z2"]:
                row = self._row(code)
                values = ["VIÑA DEL MAR", "5302"] + [row.get(field, "") for field in clean_headers]
                exported.append(values)
            workbook.save(path)

            index = _master_index(path)
            item = index["VIN ADELMAR".replace(" ", "")]
