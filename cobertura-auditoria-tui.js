(() => {
  ...
  function coverageFor(row,commune){
    ...
    return{state:"pendiente",label:"Capa disponible · cruce comunal pendiente",detail:"Forma parte del universo auditado de TUI; falta ejecutar o incorporar su matriz de intersección comunal."};
  }
  ...
  function renderTerritorial(){
    ...
    const scroll=body.closest(".capas-table-scroll");if(scroll)scroll.hidden=rows.length===0;if($("capasCoverageEmpty"))$("capasCoverageEmpty").hidden=rows.length>0;if($("capasTerritorialCount"))$("capasTerritorialCount").textContent=`${rows.length} de ${audit().capas.length} capas`;
  ...
  function configureUi(){
    const block2=document.querySelector(".capas-regions-section .capas-section-heading > div");if(block2)block2.innerHTML=`<p class="eyebrow">BLOQUE 2 · COBERTURA NACIONAL</p><h3>Estado por región y comuna</h3><p>Las comunas se evalúan contra el mismo universo maestro de <strong>${audit().capas.length} capas</strong>. Al incorporar los cruces geométricos, los conteos se actualizan sin cambiar de catálogo.</p>`;
    const block3=document.querySelector(".capas-coverage-section .capas-section-heading > div");if(block3)block3.innerHTML=`<p class="eyebrow">BLOQUE 3 · DETALLE TERRITORIAL</p><h3>Las ${audit().capas.length} capas en <span id="capasSelectedLayerTitle">la comuna</span></h3><p>El detalle usa exactamente el mismo catálogo del Bloque 1: nombre normalizado, origen, fecha y estado de auditoría, sumando la cobertura comunal cuando exista un cruce geométrico.</p>`;
    ...
    if($("capasCrossBannerTitle"))$("capasCrossBannerTitle").textContent=`Universo sincronizado · ${audit().capas.length} capas`;if($("capasCrossBannerText"))$("capasCrossBannerText").textContent="El catálogo, la cobertura nacional y el detalle territorial ya usan el mismo universo. Una capa puede estar visible aunque su cruce comunal, fuente, actualización o consolidación siga pendiente.";
  }
  ...
  function bind(){
    ...
    $("capasSearch")?.addEventListener("input",e=>{state.search=e.target.value;setTimeout(renderTerritorial,0)});$("capasCategoryFilter")?.addEventListener("change",e=>{state.category=e.target.value;setTimeout(renderTerritorial,0)});$("capasCoverageFilter")?.addEventListener("change",e=>{state.coverage=e.target.value;setTimeout(renderTerritorial,0)});$("capasSort")?.addEventListener("change",e=>{state.sort=e.target.value;setTimeout(renderTerritorial,0)});$("capasRegionSearch")?.addEventListener("input",e=>{state.regionSearch=e.target.value;setTimeout(renderRegions,0)});$("capasRegionSort")?.addEventListener("change",e=>{state.regionSort=e.target.value;setTimeout(renderRegions,0)});
