...
def _row_signature(row: dict[str, Any]) -> str:
    return json.dumps([row.get(field, "") for field in FIELDS], ensure_ascii=False, sort_keys=False, default=str)
...
def _matching_rule(row: dict[str, Any], field: str, original: Any, catalog: dict[str, Any]) -> dict[str, Any] | None:
    for rule in catalog.get("exact_rules", []):
        if _rule_key(rule.get("comuna")) != _rule_key(row.get("COMUNA")):
            continue
        if rule.get("field") != field:
            continue
        if _rule_key(rule.get("original")) != _rule_key(original):
            continue
...
def _apply_rule(row_number: int, row: dict[str, Any], field: str, value: Any, findings: list[Finding], catalog: dict[str, Any]) -> Any:
    ...
    return proposed if confidence == "ALTA" and bool(rule.get("auto_apply", True)) else value
...
def _normalize_cell(row_number: int, field: str, value: Any, findings: list[Finding]) -> Any:
    ...
    if isinstance(value, str):
        ...
        if cleaned.upper() in NULL_LITERALS:
            findings.append(Finding(row_number, field, original, "", "NORMALIZACIÓN DE FORMATO", "ALTA", "Literal de ausencia reemplazado por vacío."))
            return ""
        if cleaned != value:
            findings.append(Finding(row_number, field, original, cleaned, "NORMALIZACIÓN DE FORMATO", "ALTA", "Espacios o caracteres invisibles normalizados."))
        value = cleaned
    ...
    if field == "AGRUPAMIENTO" and value:
        ...
        if tokens and all(token in GROUP_ORDER for token in tokens):
            ...
            if proposed != str(value):
                findings.append(Finding(row_number, field, original, proposed, "NORMALIZACIÓN DE FORMATO", "ALTA", "Agrupamiento normalizado sin duplicar filas."))
            value = proposed
    ...
    if field in NUMERIC_DECIMAL and isinstance(value, str):
        ...
        if num is not None:
            if str(value) != str(num):
                findings.append(Finding(row_number, field, original, num, "NORMALIZACIÓN DE FORMATO", "ALTA", "Coeficiente convertido a valor numérico."))
            value = num

    return value


def audit_table(headers: list[str], rows: list[dict[str, Any]], rule_catalog: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    if missing:
        findings.append(Finding(0, "ESTRUCTURA", ", ".join(missing), "", "ERROR CONFIRMADO", "ALTA", "Faltan campos obligatorios."))
    if extras:
        findings.append(Finding(0, "ESTRUCTURA", ", ".join(extras), "", "POSIBLE ERROR", "MEDIA", "Existen columnas adicionales; se excluyen de la salida productiva."))
    if len(headers) == len(FIELDS) and headers != FIELDS and not missing:
        findings.append(Finding(0, "ESTRUCTURA", " | ".join(headers), " | ".join(FIELDS), "NORMALIZACIÓN DE FORMATO", "ALTA", "Los 35 campos existen pero están en otro orden."))
    ...
    for index, source_row in enumerate(rows, start=2):
        ...
        if code and (len(code) > 28 or len(re.findall(r"\s+", code)) >= 2):
            findings.append(Finding(index, "CODIGO_PRC", code, "", "POSIBLE ERROR", "MEDIA", "Parece una descripción más que un código identificador; requiere fuente oficial."))

        pisos = row.get("PISOS_MAX")
        pisos_num = _numeric(pisos)
        if pisos_num is not None and not pisos_num.is_integer():
            findings.append(Finding(index, "PISOS_MAX", pisos, "", "POSIBLE ERROR", "ALTA", "PISOS_MAX es decimal; no se trunca sin fuente oficial."))

        for field in ["CONSTRUCCION", "OCUPACION", "OCUPACION_SUP", "ALTURA_MIN", "ALTURA_MAX", "AREA_LIBRE_MIN"]:
            value = row.get(field)
            num = _numeric(value)
            if num is not None and num < 0:
                findings.append(Finding(index, field, value, "", "POSIBLE ERROR", "MEDIA", "Valor negativo que normalmente no corresponde a este parámetro."))
            if isinstance(value, str) and OCR_PATTERN.search(value):
                findings.append(Finding(index, field, value, "", "POSIBLE ERROR", "MEDIA", "Patrón compatible con OCR O/0, P/0 o I/l/1; no se autocorrige sin fuente."))

        amin = _numeric(row.get("ALTURA_MIN"))
        amax = _numeric(row.get("ALTURA_MAX"))
        if amin is not None and amax is not None and amin > amax:
            findings.append(Finding(index, "ALTURA_MIN / ALTURA_MAX", f"{row.get('ALTURA_MIN')} > {row.get('ALTURA_MAX')}", "", "ERROR CONFIRMADO", "ALTA", "ALTURA_MIN es mayor que ALTURA_MAX."))

        if re.search(r"AISLADO|PAREADO|CONTINUO", str(row.get("AREA_LIBRE_MIN", "")), re.I):
            findings.append(Finding(index, "AREA_LIBRE_MIN", row.get("AREA_LIBRE_MIN"), "", "POSIBLE ERROR", "MEDIA", "Contenido compatible con AGRUPAMIENTO; posible desplazamiento de columnas."))
        if re.search(r"AISLADO|PAREADO|CONTINUO", str(row.get("RASANTE", "")), re.I):
            findings.append(Finding(index, "RASANTE", row.get("RASANTE"), "", "POSIBLE ERROR", "MEDIA", "Contenido incompatible con rasante; posible desplazamiento de columnas."))

    seen: dict[str, int] = {}
    for index, row in enumerate(normalized_rows, start=2):
        signature = _row_signature(row)
        if signature in seen:
            findings.append(Finding(index, "FILA", f"Duplicada con fila {seen[signature]}", "", "ERROR CONFIRMADO", "ALTA", "Registro completamente duplicado."))
        else:
...
def _write_qa_xlsx(path: Path, comuna: str, findings: list[dict[str, Any]]) -> None:
    columns = ["COMUNA", "FILA", "CAMPO", "VALOR_ORIGINAL", "VALOR_NUEVO", "ESTADO", "CONFIANZA", "MOTIVO", "FUENTE", "PAGE", "REGLA"]
    workbook = Workbook()
    sheet = workbook.active
...
def process_file(input_path: str | Path, normalized_dir: str | Path, qa_dir: str | Path, rule_catalog_path: str | Path | None = None) -> dict[str, Any]:
    ...
    safe = re.sub(r"[^A-Za-z0-9]+", "_", unicodedata.normalize("NFD", comuna).encode("ascii", "ignore").decode()).strip("_").upper()
    ...
    return {**status, "normalized_path": str(normalized_path), "qa_path": str(qa_path), "status_path": str(status_path)}
